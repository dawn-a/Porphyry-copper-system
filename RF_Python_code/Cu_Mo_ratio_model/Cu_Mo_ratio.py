...
if __name__ == '__main__':
    ...
    useknn_impute = True
    file_name = 'Cu_Mo_ratio_label.xlsx'
    ...
    if not usehyparams_search:
        ...
        clf = RandomForestClassifier(
            ...
            max_depth=18,  # 搜索得到的最优值
            min_samples_split=2,  # 搜索得到的最优值
            min_samples_leaf=1,  # 搜索得到的最优值
            max_features=0.18725906778154905,  # 搜索得到的最优值
    ...
    # ---------------------------------------------------------
    # 2. 绘制图 (A): SHAP Summary Plot (点图/蜂群图)
    # ---------------------------------------------------------
    # 对于二分类问题，RF的shap_values是一个包含两个数组的列表：[Class0_shap, Class1_shap]
    # 通常我们关注正类 (Label=1) 的影响，所以取 shap_values[1]
    # 如果是多分类，请根据你的目标类别修改索引

    target_class_index = 1  # 假设 1 是我们要关注的 "阳性/风险" 类别
    shap_values_target = shap_values[:,:,0]
    print(shap_values_target)
    # 检查 shap_values 是否为列表 (RF通常返回列表)
    ...
    plt.tight_layout()
    plt.savefig('shap_summary_bar_plot.png', dpi=600)
    plt.show()
